        ...
        <div class="container-fluid" id="container-wrapper">
          ...
          <div class="row mb-5 pb-5">
            <div class="col-lg-6">
              ...
              <div class="card mb-4">
                ...
                <div class="card-body">
                  ...
                  <?php foreach ($adminofcity_info as $info){?>
                    <div class="form-group">
                        <label for="adminofcity_username">Kullanıcı Adı</label>
                        <div class="input-group mb-3">
                          <div class="input-group-prepend">
                            <span class="input-group-text" id="adminofcity_username">@</span>
                          </div>
                          <input disabled type="text" class="form-control" id="adminofcity_username" name="adminofcity_username" aria-label="Username" aria-describedby="adminofcity_username" autofocus 
                            value="<?= $info->adminofcity_username ?>" placeholder="<?= $info->adminofcity_username ?>">
                        </div>
                    </div>
                    <div class="form-group">
                      <label for="adminofcity_name">Ad</label>
                      <input type="text" class="form-control" id="adminofcity_name" name="adminofcity_name" 
                            value="<?= $info->adminofcity_name ?>" placeholder="<?= $info->adminofcity_name ?>">
                    </div>
                    <div class="form-group">
                      <label for="adminofcity_surname">Soyad</label>
                      <input type="text" class="form-control" id="adminofcity_surname" name="adminofcity_surname"
                            value="<?= $info->adminofcity_surname ?>" placeholder="<?= $info->adminofcity_surname ?>">
                    </div>
                    <div class="form-group">
                      <label for="gender_type">Cinsiyet</label>
                      <select class="form-control" id="gender_type" name="gender_type">
                        <option value="<?= $info->gender_type ?>" selected><?= $info->gender_type ?></option>
                        <option value="Erkek">Erkek</option>
                        <option value="Kadın">Kadın</option>
                      </select>
                    </div>
                    <div class="form-group">
                      <label for="adminofcity_telephone">Telefon</label>
                      <input type="text" class="form-control" id="adminofcity_telephone" name="adminofcity_telephone" maxlength="10"
                            value="<?= $info->adminofcity_telephone ?>" placeholder="<?= $info->adminofcity_telephone ?>">
                    </div>
                    <div class="form-group">
                      <label for="adminofcity_mail">Mail Adresi</label>
                      <input type="email" class="form-control" id="adminofcity_mail" name="adminofcity_mail"
                            value="<?= $info->adminofcity_mail ?>" placeholder="<?= $info->adminofcity_mail ?>">
                    </div>                             
                    
                    <button type="submit" class="btn btn-lg btn-success btn-icon-split mt-3" name="update_adminofcity_info">
            ...
            <div class="col-lg-6">
              ...
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Profil Resmini Güncelleyin!</h6>
                </div>
                <div class="card-body">
                  <form action="/hemsehrimorg/tools/process-admin.php" method="POST" enctype="multipart/form-data">
                  <?php foreach ($adminofcity_info as $adminofcity){?>
                    <img src="<?= $adminofcity->adminofcity_img ?>" style="width: 200px;" class="rounded">  
                    <br>                     
                    <div class="form-group mt-5">
                      <label for="adminofcity_img">Profil Resmi</label>
                      <input type="file" class="form-control" id="adminofcity_img" name="adminofcity_img" accept="image/*" required>
                    </div>  
                    <button type="submit" class="btn btn-lg btn-success btn-icon-split mt-3" name="adminofcity_update_pp">
                      <span class="icon text-white-100"><i class="fas fa-check"></i></span>
                      <span class="text">Tamamla</span>
                    </button>
                    <?php } ?>                        
